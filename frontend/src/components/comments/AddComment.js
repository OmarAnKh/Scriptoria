...
const AddComment = ({ storyId, updateComments, triggerCount, setTriggerCount, replyInfo, setReplyInfo, replyFlag, setReplyFlag }) => {
    const { auth } = useAuth();
    ...
    return (
        <div className='bg-light p-0 mt-0 row container rounded'>
       {
        ...
        <span>{t("Comments.reply-to")} {replyInfo.to}</span>
        </div>
