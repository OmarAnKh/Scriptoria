  ...
  const WpNavBar = ({ socket, setMode, setState }) => {
    ...
    useEffect(() => {
      const ExistStory = async () => {
        ...
        const getInvitedUserInformationResponse = await getListOfAccountsInformationByAnArrayOfIds(getInvitationsResponse);
            ...
            <div className="modal-content" style={{ display: "inline" }}>
              ...
              <div className="container gap-2 btn-group mb-2">
                <button
                  ...
                  disabled={!isOwner || addCooldown > 0}
                >
                  {addCooldown > 0 ? `Add (${addCooldown}s)` : 'Add'}
                </button>
